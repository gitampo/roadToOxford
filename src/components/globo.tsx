...
const PARALLELI = [
  { cy: 40, rx: 65, ry: 8, w: 0.4 },
  { cy: 70, rx: 84, ry: 11, w: 0.4 },
  { cy: 100, rx: 90, ry: 0.1, w: 0.7 },
  { cy: 130, rx: 84, ry: 11, w: 0.4 },
...
function PuntoOxford() {
  ...
  const alone = useAnimatedProps(() => ({
    transform: `translate(${OXFORD.cx}, ${OXFORD.cy}) scale(${0.5 + 2.5 * p.value}) translate(${-OXFORD.cx}, ${-OXFORD.cy})`,
    opacity: 0.8 * (1 - p.value * p.value),
  }));
  ...
  return (
    <>
      <AnimatedCircle
        ...
        animatedProps={alone}
      />
      <Circle cx={OXFORD.cx} cy={OXFORD.cy} r={3} fill="#ffffff" />
    </>
  );
}
